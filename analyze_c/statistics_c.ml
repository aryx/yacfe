...
let (statistics_of_program : Ast_c.program -> entities_stat) =
 fun asts -> 
  ...
  let bigf_phase1 = { Visitor_c.default_visitor_c with
    ...
    Visitor_c.kstatementseq = (fun (k,bigf) top -> 
      k top;
      match top with
      | C.StmtElem _ -> ()
      | C.CppDirectiveStmt _ -> 
          incr Stat.nDirectiveStmt;
      | C.IfdefStmt _ -> 
          incr Stat.nIfdefStmt;
      ...
      | C.DeclList (xs,ii1) -> 
          ...
          | (x,ii2)::xs -> 
              ...
              | Some ((s, optini),_ii) -> 
                  ...
                  then 
                    let _ii = Lib.ii_of_decl decl in
                    if null xs 
                    then 
    ...
    Visitor_c.ktype = (fun (k, bigf) ft -> 
      ...
      | C.StructUnion (su, sopt, fields) -> 
          ...
          fields +> List.iter (fun (xfield, iipttvirg) -> 
            ...
            | C.MacroStructDeclTodo -> 
                incr Stat.nMacroStructDecl;
                pr2 "MacroStructDeclTodo"

            | C.CppDirectiveStruct _ -> 
                incr Stat.nDirectiveStruct;

            | C.IfdefStruct dir -> 
                incr Stat.nIfdefStruct;
      ...
      | C.TypeOfType _ 
        -> 
          incr Stat.nGccTypeof;

      | C.BaseType base -> 
          (match base with
          | C.IntType (C.Si (_si, C.CLongLong)) -> 
              incr Stat.nGccLongLong;
          | _ -> ()
          )
