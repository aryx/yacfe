...
let is_comment_or_space_or_stuff = function
  ...
  | TC.TCommentCpp (Ast_c.CppPassingNormal,_)  -> true
  | TC.TCommentCpp (Ast_c.CppPassingCosWouldGetError,_)  -> true
 ...
 = fun xinfo asts haround -> 
  ...
  let bigf_phase1 = { Visitor_c.default_visitor_c with
    ...
    Visitor_c.ktoplevel = (fun (k, bigf) top -> 
      ...
      | C.Definition def -> 
          ...
          let (returnt, (paramst, (b, iib))) = defbis.C.f_type in
      ...
      | C.Include {i_include = (s, ii);} -> 
          ...
          then 
            [iinclude;iipath] +> minmax_ii_attached hboth 
              (M.Include (Ast_c.s_of_inc_file s))
    ...
    Visitor_c.kdecl = (fun (k,bigf) decl -> 
      ...
      | C.DeclList (xs,ii1) -> 
          ...
          | [] -> raise Impossible
          | (x,ii2)::xs -> 
              let {v_namei = var; v_type = returnType;
    ...
    Visitor_c.ktype = (fun (k, bigf) ft -> 
      ...
      | C.StructUnion (su, sopt, fields) -> 
          ...
          fields +> List.iter (fun (xfield, iipttvirg) -> 
            ...
            (match xfield with 
            | C.DeclarationField (C.FieldDeclList (onefield_multivars, iiptvirg)) -> 
                ...
                | [(x, ii)] -> 
                    ...
                    Ast_c.fieldname_of_fieldkind x +> Common.do_option (fun s -> 
                      iipttvirg +> max_ii_same_line_after hboth (M.Field s);
                      let ii = Lib.ii_of_struct_fieldkinds onefield_multivars in
            ...
            | C.EmptyField -> ()
            | C.MacroStructDeclTodo -> ()
            | C.CppDirectiveStruct _ -> ()
            | C.IfdefStruct _ -> ()
            );
          );
  ...
  let bigf_phase2 = { Visitor_c.default_visitor_c with
    ...
    Visitor_c.ktoplevel = (fun (k, bigf) top -> 
      k top;
      match top with
      | C.Definition (defbis, ii) -> 
          let statxs = defbis.C.f_body in
