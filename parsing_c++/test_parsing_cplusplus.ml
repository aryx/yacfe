...
(* module Unparse_c = Unparse_cplusplus *)
module Parsing_stat = Parsing_stat2
...
let test_parse_gen xs ext = 
  ...
  Flag_parsing_c.debug_cplusplus := true;
...
let test_parse_c xs = 
  test_parse_gen xs ".*\\.\\(cpp\\|cc\\|C\\)$"
let test_parse_h xs = 
  test_parse_gen xs ".*\\.\\(h\\|hpp\\)$"
let test_parse_ch xs = 
  test_parse_gen xs ".*\\.\\(h\\|hpp\\|cpp\\|cc\\|C\\)$"
...
let test_cfg file = 
  ...
  program +> List.iter (fun (e,_) -> 
    let _toprocess = 
      ...
      | None, _ -> true
      | Some s, Ast_c.TopDecl (Ast_c.Definition (((funcs, _, _, c),_)),_noii)  -> 
          s = funcs
      | _, _ -> false 
    in
          
(* comment for parsing_c++:
    if toprocess
    then 
