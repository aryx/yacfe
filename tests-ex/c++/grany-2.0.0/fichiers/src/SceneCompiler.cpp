...
void SceneCompiler::yyerror(char *m)
{ 
  /*
...
Scene* SceneCompiler::parseStream(istream& is)
{
  _scanner.yyin = &is;
  yyparse();
...
int SceneCompiler::parseStreamWithResults(istream& is, Scene** scene)
{
  _scanner.yyin = &is;
...
Scene * SceneCompiler::parseFichier(string nom_fichier)
{
