...
struct bin_str trouve_extension(char *name)
{
...
COLOR couleur_de_bin_str(struct bin_str val)
{
  if ((val.len!=5)||
      (val.string[2]!=';')||
...
/* non utilise mais bon */
#define trouve_couleur(s) (couleur_de_bin_str(trouve_extension(s)))


/* fonction exporte */
COLOR color_of_extesion(char *chaine)
{
  return couleur_de_bin_str(trouve_extension(chaine));
}
